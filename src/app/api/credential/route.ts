...
export async function POST() {
  ...
  const randomIndex = Math.floor(Math.random() * exampleCredentials.length)
  const exampleCredential = exampleCredentials[randomIndex]

  return Response.json(exampleCredential)
}
